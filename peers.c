					...
					uint8_t *peer_id)
{
	if ( (tracer_addr->ai_family != AF_INET) && 
		...
		while (!peer_list_obtained) {
			...
			memset(&(announce_request[84]), 4, 0);
			for (int i=0; i<4; ++i) {
...
void *peer_discovery(void *thread_data)
{
	...
	for (int i=0; i<NoOfTrackers; ++i) {
		struct tracker *curr_tracker = vector_read(ptd->trackers_vector,
				                           i);
		obtain_peers_from_tracker(curr_tracker, ptd->peers_vector, ptd->access_peers_vector_mutex, ptd->info_dict_sha1_hash, ptd->peer_id);
	}
}
