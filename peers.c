...
void fill_peer_discovery_thread_structures(struct peer_sdiscovery_thread_independent_data *pdt,
                                           int NoOfTrackers,
					   ...
					   pthread_mutex_t *access_peers_tree_mutex)
{
	for (int i=0; i<no_of_peer_discovery_threads; ++i) {
                ...
                if (base_no_of_trackers > 0) {
                        if (i<(no_of_peer_discovery_threads - 1)) {
                                for (int j=0; j<base_no_of_trackers; ++j) {
                                        ...
                                        vector_push_back( (ptd[i]).trackers_vector, curr_tracker);
                                }
                        }
                        else {
        ...
        for (int i=0; i<no_of_peer_discovery_threads; ++i) {
                (ptd[i]).info_dict_sha1_hash = info_hash;
		(ptd[i]).peer_id = peer_id;
		(ptd[i]).peers_tree = peers_tree;
		(ptd[i]).access_peers_tree_mutex = access_peers_tree_mutex; 
        }
	...
	if ( (tracer_addr->ai_family != AF_INET) && 
		...
		while (!peer_list_obtained) {
			...
			if (ip_version == 4) {
				...
				uint8_t *trk_v6_addr = (((struct sockaddr_in6 *)tracker_addr->ai_addr)->sin6_addr).s6_addr;
                                printf("Received connect response from tracker %x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x..\n", trk_v6_addr[0], trk_v6_addr[1],
                                                                                                                                trk_v6_addr[2], trk_v6_addr[3],
                                                                                                                                trk_v6_addr[4], trk_v6_addr[5],
                                                                                                                                trk_v6_addr[6], trk_v6_addr[7],
                                                                                                                                trk_v6_addr[8], trk_v6_addr[9],
			...
			while (1) {
				...
				if (ip_version == 4) {
					...
					uint8_t *trk_v6_addr = (((struct sockaddr_in6 *)tracker_addr->ai_addr)->sin6_addr).s6_addr;
					printf("Received and validated announce response from tracker %x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x..\n", trk_v6_addr[0], trk_v6_addr[1],
																	trk_v6_addr[2], trk_v6_addr[3],
																	trk_v6_addr[4], trk_v6_addr[5],
																	trk_v6_addr[6], trk_v6_addr[7],
																	trk_v6_addr[8], trk_v6_addr[9],
