 ...
 * compile: gcc -I ~/Documents/Containers/C -c bittorrent.c
 * link: gcc bittorrent.o -o bittorrent ~/Documents/Containers/C/vector.o ~/Documents/Containers/C/doubly_linked_list.o ~/Documents/Containers/C/binary_tree.o -lm -lpthread
 ************************************/
...
uint16_t obtain_tracker_port(char *ml, int len)
{
	int colon_count = 0;
	char colon0[3] = {'%','3','a'};
        char colon1[3] = {'%','3','A'};
	...
	while (index <= (len-3)) {
		if ( (strncmp(&(ml[index]), colon0, 3)==0) || 
			...
			if (colon_count == 2) {
				index += 3;
				uint16_t port_no = atoi(&(ml[index]));
				return port_no;
			}
			index += 3;
		}
...
{
	struct vector *trackers = vector_null_init(sizeof(struct tracker_endpoint), print_unsigned_int);
	...
	while (curr_pos < ml_str_len) {
		if (strncmp(&(ml[curr_pos]), info_present, 4)==0) {
			...
			if ((curr_pos + curr_tracker_info_index+3)>=ml_str_len) {
				...
				uint16_t tracker_udp_port = obtain_tracker_port(&(ml[curr_pos]), ml_str_len - curr_pos);
				...
				uint16_t tracker_udp_port = obtain_tracker_port(&(ml[curr_pos]), curr_tracker_info_index);
				struct tracker_endpoint te = {tracker_url, tracker_udp_port};
				vector_push_back(trackers, &te);
				curr_pos += curr_tracker_info_index;
				continue;
...
struct tracker_announce_response_header
{
	uint32_t action;
	uint32_t transaction_id;
	uint32_t interval;
...
uint8_t magic_constant[8] = {0x00, 0x00, 0x04, 0x17, 0x27, 0x10, 0x19, 0x80};
...
void init_tracker_addr_hint(struct addrinfo* tracker_addr_hint)
{
...
int sockaddr_cmp(struct sockaddr_in* a0, struct sockaddr_in* a1, socklen_t len)
{
	if (len != sizeof(struct sockaddr_in)) {
		printf("Error in expected return address length after recvfrom call.\n");
		return 1;
	}
	if ( ((a0->sin_family) == (a1->sin_family)) &&
...
int receive_conn_response(void* conn_response,int client_socket_fd, struct addrinfo* tracker_addr_info, int* n, fd_set* read_fds, struct timeval* tv)
{
	...
	while (bytes_recv < 16) {
		...
		if (sockaddr_cmp(&src_addr, tracker_addr, addr_len)==0) {
	...
	return resend_conn_request;
}

int init_tracker_announce_request(int client_socket_fd,
				   struct tracker_announce_request* tar,
				   struct tracker_conn_response* tcr,
				   struct obtain_peer_list_data* tm)
{
	memset(tar, 0, sizeof(struct tracker_announce_request));
	memcpy(tar->connection_id, tcr->connection_id, 8);
	tar->action = ntohl(1);
	tar->transaction_id = rand() + rand();
	extract_info_hash(tm->magnet_link, tar->info_hash);
	insert_random_peer_id(tar->peer_id);
	memcpy(tm->peer_id, tar->peer_id, 20); // save peer id for later
	memset(tar->downloaded, 0 ,8); // Downloaded set to zero
	tar->event = 0;
	tar->ip_addr = 0;
	tar->num_want = 0xffffffff;
	...
	tar->port = client_socket_addr.sin_port;
	return 0;
}
...
void obtain_peer_list_from_tracker(void *tracker_metadata)
{
	...
	init_tracker_addr_hint(&tracker_addr_hint); 	
        ...
        uint8_t temp_peer_list[6020]; // temp buffer fits max 1000 peers in
	...
	struct addrinfo *tracker_addr_info = 0;
        ...
        if (getaddrinfo(curr_tracker->url, tracker_port, &tracker_addr_hint,
                                      &tracker_addr_info) != 0 ) {
	...
	while (tracker_addr_info) {
                        ...
                        if (res == -1) {
                        	struct sockaddr_in *tracker_addr = (struct sockaddr_in *)(tracker_addr_info->ai_addr);
                                ...
                                continue;
                                        ...
                                        // timeout occurred
                                printf("Timeout occurred after %d seconds. Expected data from %s:%s\n", get_timeout(n), curr_tracker->url, tracker_port);
                                ++n;
				continue;
                        }
                        else if (res > 0) {
				...
				int resend_conn_request = receive_conn_response(conn_response, client_socket_fd, tracker_addr_info, &n, &read_fds, &tv);
				if (resend_conn_request) {
					continue;
				}
				struct tracker_conn_response *tcresponse = (struct tracker_conn_response *)conn_response;
				if (tcresponse->action != 0) {
					printf("Error in action field of tracker connect response from tracker %s:%s\n", curr_tracker->url, tracker_port);
					continue;
				}
				...
				if (init_tracker_announce_request(client_socket_fd, 
							          &tarequest, 
								  tcresponse, 
								  tm) != 0) {
					return 0;
				...
				while ((difftime(connection_id_recv, curr_time) < 60.0) && (n<=8)) {
					...
					if (res == 0) {
					       	printf("Timeout occurred (after %d secs)  after sending tracker %s:%s announce "
							"request.\n", get_timeout(n), curr_tracker->url, tracker_port);
						++n;
						curr_time = time(NULL);
						continue;
						...
						while (bytes_recv < 20) {
							...
							int new_bytes = recvfrom(client_socket_fd,
										&(temp_peer_list[bytes_recv]), 6020-bytes_recv, 0,
										(struct sockaddr *)&src_addr, &addr_len);
							if (sockaddr_cmp(&src_addr, 
									  tracker_addr,
									  sizeof(struct sockaddr_in))==0) {
								bytes_recv += new_bytes;
								printf("read %d bytes from %s:%s trackers announce response.\n",bytes_recv, curr_tracker->url, tracker_port);
							}
							else {
								continue;	
							}
						...
						struct tracker_announce_response_header* tarh = (struct tracker_announce_response_header *)temp_peer_list;
						if (ntohl(tarh->action) != 1) {
							uint8_t* val = &(tarh->action);
							printf("acion: %u %u %u %u\n", *(val), *(val+1), *(val+2), *(val+3));
							printf("tracker %s:%s announce response action field is not 1.\n", curr_tracker->url, tracker_port);
							curr_time = time(NULL);
                                                        continue;
						}	
						if (tarh->transaction_id != tarequest.transaction_id) {
							printf("Error in transaction id received in announce response from tracker %s:%s.\n", curr_tracker->url, tracker_port);
							curr_time = time(NULL);
							continue;
						}
						if (bytes_recv > 20) {
							printf("bytes received in announce response from tracker %s:%s : %d\n", curr_tracker->url, tracker_port, bytes_recv);
							tm->list_len = (bytes_recv - 20)/6;
							tm->peer_list = (uint8_t*)malloc(bytes_recv);
							memcpy(tm->peer_list, temp_peer_list, bytes_recv);
						}
						else {
							tm->list_len = 0;
							tm->peer_list = 0;
...
int get_max_peer_list_index(struct obtain_peer_list_data *pl_metadata, int NoOfTrackers)
{
	uint32_t max_len = 0;
 	int32_t index = 0;
	for (uint32_t i=0; i<NoOfTrackers; ++i) {
...
void retrieve_metadata_info(unsigned char *buff, struct metadata_info *mdi)
{
	...
	mdi->ut_metadata = extract_ut_metadata(&(buff[m_dictionary_index])); // correct up tohere
...
void *perform_data_exchange(void *per_thread_data)
{
	...
	int client_socket_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); // TCP socket
        if (client_socket_fd == -1) {
                printf("Error obtaining tcp/ip socket client endpoint.\n");
        }
...
{ 
	...
	srand(time(NULL));
	uint64_t magic_constant = 0x0000041727101980;
	...
	uint8_t peer_list[(max_list_len*6)+20];
	...
	for (int i=0; i<NoOfTrackers; ++i) {
		if (((pl_metadata[i]).peer_list) != 0) {
			free((pl_metadata[i]).peer_list);
		}
	}
	free(pl_metadata);
	printf("Peer list obtained (containing %d peers)\n", max_list_len); // performance good up to here
	...
	free(obtain_md_info_per_thread_data); // performance good up to here
	...
	*received_subpieces = (struct binary_tree *)init_btree(sizeof(unsigned int), compare_subpiece_index, print_subpiece_index);
	...
	for (int i=0; i<max_list_len; ++i) {
		...
		(thread_data[i]).cs.download_rates = vector_null_init(sizeof(unsigned int), print_unsigned_int);
			...
			if (count < 3) {
				if (optimistic_unchoke_peer_index == -1) {
					if (!interested_peers) {
						...
						struct vector *unchoke_peer_indices_list = vector_null_init(sizeof(unsigned int), print_unsigned_int);
